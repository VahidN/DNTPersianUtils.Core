...
public static class EquinoxCalculator
{
    ...
    public static DateTime GetSunEventUtc(this int year, SunEvent sunEvent)
    {
        ...
        if (year >= 1000)
        {
            ...
            switch (sunEvent)
            {
                case SunEvent.VernalEquinox:
                    julianEphemerisDay = 2451623.80984 + 365242.37404 * y + 0.05169 * (y * y) - 0.00411 * (y * y * y) -
                                         0.00057 * (y * y * y * y);

                    break;
                case SunEvent.SummerSolstice:
                    julianEphemerisDay = 2451716.56767 + 365241.62603 * y + 0.00325 * (y * y) - 0.00888 * (y * y * y) -
                                         0.00030 * (y * y * y * y);

                    break;
                case SunEvent.AutumnalEquinox:
                    julianEphemerisDay = 2451810.21715 + 365242.01767 * y + 0.11575 * (y * y) - 0.00337 * (y * y * y) -
                                         0.00078 * (y * y * y * y);

                    break;
                case SunEvent.WinterSolstice:
                    julianEphemerisDay = 2451900.05952 + 365242.74049 * y + 0.06223 * (y * y) - 0.00823 * (y * y * y) -
                                         0.00032 * (y * y * y * y);
        ...
        else
        {
            ...
            {
                case SunEvent.VernalEquinox:
                    julianEphemerisDay = 1721139.29189 + 365242.13740 * y + 0.06134 * (y * y) - 0.00111 * (y * y * y) -
                                         0.00071 * (y * y * y * y);

                    break;
                case SunEvent.SummerSolstice:
                    julianEphemerisDay = 1721233.25401 + 365241.72562 * y + 0.05323 * (y * y) - 0.00907 * (y * y * y) -
                                         0.00025 * (y * y * y * y);

                    break;
                case SunEvent.AutumnalEquinox:
                    julianEphemerisDay = 1721325.70455 + 365242.49558 * y + 0.11677 * (y * y) - 0.00297 * (y * y * y) -
                                         0.00074 * (y * y * y * y);

                    break;
                case SunEvent.WinterSolstice:
                    julianEphemerisDay = 1721414.39987 + 365242.88257 * y + 0.00769 * (y * y) - 0.00933 * (y * y * y) -
                                         0.00006 * (y * y * y * y);
    ...
    public static bool IsStartOfNewYear(this DateTime now, double utcDelta = 3.5)
    {
        ...
        return now.Year == vernalEquinoxDateTime.Year && now.Month == vernalEquinoxDateTime.Month &&
               now.Day == vernalEquinoxDateTime.Day && now.Hour == vernalEquinoxDateTime.Hour &&
    ...
    public static bool IsStartOfNewYear(this DateOnly now, double utcDelta = 3.5)
        => now.ToDateTime().IsStartOfNewYear(utcDelta);
#endif
    ...
    public static DateTime JulianToUtcDate(double julianDay)
    {
        ...
        else if (ApproxEquals(e, d2: 14) || ApproxEquals(e, d2: 15))
        {
            month = (int)(e - 13.0);
        }
        else
        {
            throw new NotSupportedException(message: "Illegal month calculated.");
        }
        ...
        {
            throw new NotSupportedException(message: "Illegal year calculated.");
        }

        var span = TimeSpan.FromDays(day);

        return new DateTime(year, month, (int)day, span.Hours, span.Minutes, span.Seconds, span.Milliseconds,
            DateTimeKind.Utc);
    }
    ...
    private static double getSumOfPeriodicTerms(double julianCenturies)
        => 485 * Math.Cos(Deg2Radian * 324.96 + Deg2Radian * (1934.136 * julianCenturies)) +
