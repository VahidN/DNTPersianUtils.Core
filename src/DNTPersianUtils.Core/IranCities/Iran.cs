...
public static class Iran
{
    ...
    /// لیست استان‌ها.
    /// </summary>
    public static IEnumerable<string> IranProvinces { get; } = new List<string>
                                                               {
                                                                   "آذربایجان شرقی",
                                                                   "آذربایجان غربی",
                                                                   "اردبیل",
    ...
    /// </summary>
    public static IEnumerable<City> FindCitiesOfSelectedDistrict(string? selectedProvince, string? selectedCounty,
    ...
    private static List<Province> getProvinces()
    {
        ...
        {
            foreach (var thisProvince in Cities.Where(x => string.Equals(x.ProvinceName, province.ProvinceName,
            ...
            foreach (var county in province.Counties)
            {
                foreach (var thisCounty in Cities.Where(x =>
                                                            string.Equals(x.ProvinceName, province.ProvinceName,
                                                                          StringComparison.Ordinal)
                                                            && string.Equals(x.CountyName, county.CountyName,
                                                                             StringComparison.Ordinal)))
                {
            ...
            foreach (var county in province.Counties)
            {
                foreach (var district in county.Districts)
                {
                    foreach (var thisDistrict in Cities.Where(x =>
                                                                  ...
                                                                  string.Equals(x.CountyName, county.CountyName,
                                                                                StringComparison.Ordinal) &&
                                                                  string.Equals(x.DistrictName, district.DistrictName,
                                                                                StringComparison.Ordinal)))
                    {
